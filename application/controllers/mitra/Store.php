...
class Store extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('mitra/store/list', $store_data);
    ...
    public function create_toko()
    {
        ...
        $config['upload_path']          = './public/uploads/toko/';
        ...
        $this->form_validation->set_rules('res_name', 'toko name', 'trim|required');
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                if ($this->upload->do_upload('image')) {
                    ...
                    $this->session->set_flashdata('res_success', 'toko added successfully');
                    redirect(base_url() . 'mitra/store/index');
                } else {
                    //we got some errors
                    ...
                    $data['cats'] = $cat;
                    $this->load->view('mitra/partials/header');
                    $this->load->view('mitra/store/add_res', $data);
                    $this->load->view('mitra/partials/footer');
                }
            } else {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_FILES['image']['name'])) {
                ...
                if ($this->upload->do_upload('image')) {
                    ...
                    if (file_exists('./public/uploads/toko/' . $store['img'])) {
                        unlink('./public/uploads/toko/' . $store['img']);
                    }

                    if (file_exists('./public/uploads/toko/thumb/' . $store['img'])) {
                        unlink('./public/uploads/toko/thumb/' . $store['img']);
                    }

                    $this->session->set_flashdata('res_success', 'toko updated successfully');
                    redirect(base_url() . 'mitra/store/index');
                } else {
                    //we got some errors
                    $error = $this->upload->display_errors("<p class='invalid-feedback'>", "</p>");
                    ...
                    $data['cats'] = $cat;
                    $this->load->view('mitra/partials/header');
                    $this->load->view('mitra/store/edit', $data);
                    $this->load->view('mitra/partials/footer');
                }
            } else {
    ...
    public function delete($id)
    {
        ...
        if (empty($store)) {
            $this->session->set_flashdata('error', 'toko not found');
